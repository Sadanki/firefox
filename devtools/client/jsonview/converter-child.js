...
/**
 * Refactored Converter as ES6 Class
 */
class Converter {
  constructor() {
    this.listener = null;
...
function fixSave(request) {
  ...
  const originalType = match ? match[1] : "application/json";
...
function initialHTML(doc) {
  ...
  if (platform.startsWith("WINNT")) os = "win";
  else if (platform.startsWith("Darwin")) os = "mac";
  else os = "linux";
  ...
  return (
    ...
    element(
      "html",
      { platform: os, class: "theme-" + getTheme(), dir: Services.locale.isAppLocaleRTL ? "rtl" : "ltr" },
      [
        element("head", {}, [
          element("meta", { "http-equiv": "Content-Security-Policy", content: kCSP }),
          element("link", { rel: "stylesheet", type: "text/css", href: "chrome://devtools-jsonview-styles/content/main.css" }),
        ]),
        element("body", {}, [
          element("div", { id: "content" }, [element("div", { id: "json" })]),
          element("script", { src: baseURI + "json-viewer.mjs", type: "module", async: "true" }),
        ]),
      ]
    ).outerHTML
  );
...
function onContentMessage(e) {
  const win = this;
  if (win !== e.target) return;
  const value = e.detail.value;
  if (e.detail.type === "save") {
    win.docShell.messageManager.sendAsyncMessage("devtools:jsonview:save", value);
  }
}

// ---------- Export ----------
exports.JsonViewService = {
  createInstance: () => new Converter(),
};
